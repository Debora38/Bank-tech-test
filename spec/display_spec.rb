...
RSpec.describe Display do
  ...
  context "#print_statement" do
    ...
    it 'should print out the history of my account: multiple transactions' do
      account_history = double('account_history')
      display = Display.new(account_history)
      allow(account_history).to receive(:all_transactions).and_return([{date: '04/02/2019', credit: '', debit: display.twodecimal(500), balance: display.twodecimal(1500)}, {date: '04/02/2019', credit: display.twodecimal(2000), debit: '', balance: display.twodecimal(2000)}])
